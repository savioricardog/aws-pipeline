...
"""
Script para processar relatórios consolidados mensais em Excel e exportar para Parquet.

O script realiza:
- Padronização de nomes de colunas
- Remoção de linhas e colunas vazias
- Conversão de formato Excel para Parquet
- Criação das camadas em pastas (partições) do datalake
"""
...
def _salvar_como_parquet_local(
    df: pd.DataFrame, aba_nome: str, ano: str, mes: str
) -> None:
    """
    Salva DataFrame como arquivo Parquet na estrutura de pastas padronizada.

    Estrutura: trusted/{aba}/{ano}/{mes}/data.parquet
    ...
    Examples:
        ...
        >>> _salvar_como_parquet_local(df, 'Posição - ETF', '2025', '11')  # doctest: +SKIP
        ✓ Salvo: trusted/posicao-etf/year=2025/month=11/data.parquet
    """

    aba_padronizada = _padronizar_nome_aba(aba_nome)

    # TODO: TEMPORÁRIO! Essa função será substuída pela _salvar_como_parquet_s3 abaixo
    # Caminho do arquivo
    pasta_destino = Path(f"trusted/{aba_padronizada}/year={ano}/month={mes}")

    # Cria a estrutura de pastas caso ela não exista
    pasta_destino.mkdir(parents=True, exist_ok=True)
    arquivo_destino = pasta_destino / "data.parquet"

    # Salva parquet localmente
    df.to_parquet(arquivo_destino, index=False)

    print(f"✓ Salvo: {arquivo_destino}")
...
def _salvar_como_parquet_s3(
    ...
    Examples:
        >>> import pandas as pd
        >>> df = pd.DataFrame({'col1': [1, 2], 'col2': [3, 4]})
        >>> _salvar_como_parquet_s3(df, 'Posição - Fundos', '2025', '12')  # doctest: +SKIP
    ...
    key = f"trusted/{aba_padronizada}/year={ano}/month={mes}/data.parquet"
...
def processar_relatorio(caminho_arquivo: str) -> None:
    ...
    for aba in ABAS:
        ...
        try:
            ...
            # Salva resultado
            _salvar_como_parquet_local(df_processado, aba, ano, mes)
